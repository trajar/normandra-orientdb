...
public class OrientNonBlockingNodeQuery implements Iterable<Node> {
    private final OrientGraph graph;

    private final OrientNonBlockingGraphQuery delegate;

    public OrientNonBlockingNodeQuery(final OrientGraph db, final String query, final Collection<?> params) {
        if (null == db) {
    ...
    public OrientNonBlockingNodeQuery(final OrientGraph db, final String query, final Map<String, Object> params) {
        if (null == db) {
            throw new NullArgumentException("database");
        }
        if (null == query) {
        ...
        return new Iterator<Node>() {
            ...
            public Node next() {
                ...
                if (element instanceof OrientVertex) {
                    ...
                    final EntityMeta meta = graph.getMeta().getNodeMeta(type);
                    if (null == meta) {
